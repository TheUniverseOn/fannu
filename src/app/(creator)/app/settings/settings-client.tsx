...
import { updateCreatorProfile, updateCreatorBookingSettings } from "@/lib/actions/creators";
...
export function SettingsClient({ creator }: SettingsClientProps) {
  ...
  const [avatarUrl, setAvatarUrl] = useState<string | null>(creator.avatar_url);
  const [profileSaved, setProfileSaved] = useState(false);

  // Booking state
  const [bookingsEnabled, setBookingsEnabled] = useState(creator.booking_enabled);
  const [depositPercent, setDepositPercent] = useState(creator.default_deposit_percent);
  const [depositRefundable, setDepositRefundable] = useState(creator.default_deposit_refundable);
  const [additionalTerms, setAdditionalTerms] = useState(creator.default_additional_terms || "");
  const [bookingSaved, setBookingSaved] = useState(false);

  // Notification state (mock for now)
  const [notifyNewBooking, setNotifyNewBooking] = useState(true);
  const [notifyNewVIP, setNotifyNewVIP] = useState(true);
  ...
  const handleSaveBooking = () => {
    startTransition(async () => {
      const result = await updateCreatorBookingSettings({
        creatorId: creator.id,
        bookingEnabled: bookingsEnabled,
        defaultDepositPercent: depositPercent,
        defaultDepositRefundable: depositRefundable,
        defaultAdditionalTerms: additionalTerms || null,
      });
          ...
          <SettingsCard title="Default Terms" description="These terms will be included in all booking quotes.">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <Button onClick={handleSaveBooking} disabled={isPending}>
                  {isPending ? (
                    <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</>
                  ) : (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-4 rounded-lg border border-border">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{creator.email || "Not set"}</p>
                </div>
      ...
      {activeTab === "links" && (
        <div className="space-y-6">
          <SettingsCard title="Your Public Links" description="Share these links with your fans and clients.">
            <div className="space-y-4">
              <CopyField label="Profile Page" value={`${baseUrl}/c/${creator.slug}`} />
              <CopyField label="Booking Page" value={`${baseUrl}/book/${creator.slug}`} />
            </div>
          </SettingsCard>
